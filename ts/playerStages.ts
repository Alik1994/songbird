...
function playStage(elements: IPlayerElements, playBtn: HTMLElement): number {
  ...
  let timerId = setInterval(function () {
    ...
    if (elements.timeMarker >= elements.songDuration) {
      ...
      let minutes: number = Math.floor(elements.timeMarker / 60);
      let seconds: number = Math.floor(elements.timeMarker - minutes * 60);

      elements.timeStartMarker.textContent = `${
        minutes < 10 ? "0" + minutes : minutes
      }:${seconds < 10 ? "0" + seconds : seconds}`;

      //3. Меняем заливку тайм-бара
      elements.timeProgress.style.backgroundImage = `linear-gradient(to right,#00bc8c 0%, #008966 ${
        (elements.timeMarker / elements.songDuration) * 100
      }%, #999 ${
        (elements.timeMarker / elements.songDuration) * 100
      }%, #999 100%`;
      ...
      if (
        elements.timeMarker * (elements.timebarWidth / elements.songDuration) >=
...
function dragAndDrop(event: any, elements: IPlayerElements) {
  ...
  function moveAt(pageX: any) {
    ...
    elements.timeProgress.style.backgroundImage = `linear-gradient(to right,#00bc8c 0%, #008966 ${
      (newCoords / elements.timeProgress.offsetWidth) * 100
    }%, #999 ${
      (newCoords / elements.timeProgress.offsetWidth) * 100
    }%, #999 100%`;
